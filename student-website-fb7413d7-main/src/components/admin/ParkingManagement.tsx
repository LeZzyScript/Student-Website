...
// Generate A1-E9 parking grid
const generateParkingGrid = (): ParkingSpot[] => {
  ...
  rows.forEach((row) => {
    for (let i = 1; i <= 9; i++) {
      ...
      const isOccupied = ["A1", "B3", "C5", "D2", "E7"].includes(id);
      spots.push({
        id,
        occupied: isOccupied,
        student: isOccupied ? {
...
export function ParkingManagement() {
  const [spots, setSpots] = useState<ParkingSpot[]>(generateParkingGrid);
  ...
  const toggleOccupancy = (spotId: string) => {
    ...
    toast({
      title: "Spot Updated",
      description: `Parking spot ${spotId} has been updated. User will be notified.`,
    });
  };
  ...
  const handleRemoveOccupant = () => {
    ...
    toast({
      ...
      description: `${removeSpot.student?.name} has been removed from spot ${removeSpot.id}. They will be notified.`,
    });
    setRemoveSpot(null);
    setSelectedSpot(null);
  };
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedSpot?.occupied 
                ? "View occupant details or remove them from this spot." 
